...
vim.keymap.set({ "n", "x" }, "<leader><s-up>", function()
    require("multicursor-nvim").lineSkipCursor(-1)
end, { desc = "Skip cursor above" })
vim.keymap.set({ "n", "x" }, "<leader><s-down>", function()
    require("multicursor-nvim").lineSkipCursor(1)
...
mcos.setup({})
vim.keymap.set({ "n", "v" }, "gms", mcos.opkeymapfunc, {
    desc = "Multicursor visual",
    expr = true,
})
vim.keymap.set("n", "gmss", mcos.bufkeymapfunc, { desc = "Multicursor entire buffer" })
